...
def render_cronometro_js(
    ...
    Además, si se pasan lock_aria_label / submit_button_text, apenas el
    reloj llega a cero:
      1) Le hace blur al textarea (fuerza a Streamlit a sincronizar lo
         último que se escribió) y lo pone en solo-lectura visualmente
         "apagado", para que el teclado quede bloqueado al instante en el
         navegador, sin esperar el próximo rerun del servidor.
      2) Simula un clic sobre el botón "Finalizar" para que la corrección
         se dispare enseguida en vez de esperar el autorefresh de respaldo
         (que igual sigue estando como red de seguridad por si esto falla).
    """
    inicio_ms = int(inicio_epoch * 1000)
    lock_label_js = json.dumps(lock_aria_label) if lock_aria_label else "null"
    submit_text_js = json.dumps(submit_button_text) if submit_button_text else "null"
    html = f"""
    ...
    (function() {{
        ...
        const submitText = {submit_text_js};
        let bloqueado = false;
        ...
        function bloquearYEnviar() {{
                ...
                if (submitText) {{
                    setTimeout(function() {{
                        ...
                        for (const b of botones) {{
                            if (b.innerText && b.innerText.trim() === submitText) {{
                                b.click();
                                break;
                            }}
                        }}
            ...
            }} catch (e) {{ /* si el navegador bloquea el acceso al frame padre, queda el respaldo del backend */ }}
        }}
    ...
    if st.session_state["orto_activa"]:
        ...
        if st.session_state["orto_pausada"]:
            ...
            render_cronometro_js(
                ...
                submit_button_text="⏹️ Finalizar y corregir",
            )
        ...
        # NOTA: se sacó el st_autorefresh de respaldo que había acá. El
        # cronómetro JS ya bloquea el textarea y dispara el clic en
        # "Finalizar" apenas llega a cero (ver render_cronometro_js). Tener
        # los dos mecanismos en simultáneo generaba reruns superpuestos y
        # React tiraba "NotFoundError: removeChild" en el navegador. El
        # respaldo real que queda es este chequeo de Python, que igual
        # corrige el resultado en cualquier rerun posterior (por ejemplo,
        # si el usuario toca cualquier otra cosa) aunque el clic por JS
        # fallara.
...
# =============================================================================
    ...
    if not st.session_state["dacti_iniciada"] and not st.session_state["dacti_finalizada"]:
        ...
        render_cronometro_js(
            ...
            submit_button_text="⏹️ Finalizar",
        )
        ...
        if restante <= 0:
            ...
            # NOTA: se sacó el st_autorefresh de respaldo que había acá, por
            # el mismo motivo que en Ortografía: competía con el clic
            # automático que dispara el cronómetro JS y provocaba el error
            # "NotFoundError: removeChild" en el navegador. El chequeo de
            # Python de más arriba (if restante <= 0) sigue actuando como
            # red de seguridad en cualquier rerun posterior.
